...
const SMATable = lazy(() => import('../SMATable'));

interface TeknikalViewProps {
  viewId: ViewId;
}

export default function TeknikalView({ viewId }: TeknikalViewProps) {
  const { t } = useTranslation();
  const isSMA100 = viewId === 'teknikal-sma-100';
  const { data, loading, error } = useSMAData();
  
  const getViewTitle = () => {
    const titles: Partial<Record<ViewId, string>> = {
      'teknikal-sma-100': t('navigation.sma100'),
    };
    return titles[viewId] || t('navigation.teknikal');
  };
        ...
        <div className="w-full flex flex-col flex-1 min-h-0">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 sm:mb-8 flex-shrink-0 gap-4">
            <div>
              ...
              <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 font-medium">Teknisk analys med moving averages</p>
            </div>
          </div>
          <div className="flex-1 min-h-0 transition-all duration-300 ease-in-out">
            <Suspense fallback={<TableSkeleton rows={10} columns={4} hasStickyColumns={false} />}>
              <SMATable data={data} loading={loading} error={error} />
            </Suspense>
          </div>
        </div>
      </div>
