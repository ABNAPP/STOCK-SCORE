...
// For production, consider using Google Sheets API v4 or a serverless function
const CORS_PROXIES = [
  'https://api.allorigins.win/raw?url=',
  'https://corsproxy.io/?',
];

// Type for CSV rows parsed by Papa.parse with header: true
...
async function fetchCSVData<T>(
  csvUrl: string,
  dataTypeName: string,
  transformer: (results: Papa.ParseResult<CSVRow>) => T[],
...
function getValue(possibleNames: string[], row: CSVRow): string {
...
function getValueAllowZero(possibleNames: string[], row: CSVRow): string {
  for (const name of possibleNames) {
    // Try exact match first
...
export async function fetchBenjaminGrahamData(
  ...
  return fetchCSVData<BenjaminGrahamData>(
    ...
    'Benjamin Graham',
    (results) => {
      const benjaminGrahamData: BenjaminGrahamData[] = results.data
        .map((row: CSVRow) => {
        ...
        .filter((data: BenjaminGrahamData | null): data is BenjaminGrahamData => data !== null);
...
export async function fetchScoreBoardData(
  ...
  return fetchCSVData<ScoreBoardData>(
    ...
    'Score Board',
    (results) => {
      const scoreBoardData: ScoreBoardData[] = results.data
        .map((row: CSVRow) => {
        ...
        .filter((data: ScoreBoardData | null): data is ScoreBoardData => data !== null);
